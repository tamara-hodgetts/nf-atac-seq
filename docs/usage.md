# nf-core/thatacseq: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/thatacseq/usage](https://nf-co.re/thatacseq/usage)
...
```console
nextflow run nf-core/thatacseq --input samplesheet.csv --genome GRCh37 -profile docker
```
...
```console
nextflow pull nf-core/thatacseq
```
...
First, go to the [nf-core/thatacseq releases page](https://github.com/nf-core/thatacseq/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
